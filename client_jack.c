 ...
 * @LastEditTime: 2021-05-18 17:46:40
...
void write_log(char *info, int mode) {
    if (mode == 0) {
        FILE *fp = fopen("./jack.txt", "w");
        fprintf(fp, "log: %s\n", info);
        fclose(fp);

    } else {
        FILE *fp = fopen("./jack.txt", "a");
        fprintf(fp, "log: %s\n", info);
        fclose(fp);
    }
}
...
int main(int argc, char** argv) {
    ...
    int count = 0;
    while (1) {
        int fd = open("./AnotherDomain/jack_RD", O_RDONLY);
        unsigned char buffer[2048] = {0};
        int ret = read(fd, buffer, 2048);
        if (ret > 0) {
            //printf("got %d\n", buffer[0]);
            if (buffer[0] == 3) {
                //printf("got message 3\n");
                ...
                int fd_wr = open("./AnotherDomain/jack_WR", O_WRONLY);
                char temp[2048];
                temp[0] = 6;
                temp[1] = 0;
