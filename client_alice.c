...
void write_log(char *info, int mode) {
    if (mode == 0) {
        FILE *fp = fopen("./alice.txt", "w");
        fprintf(fp, "log: %s\n", info);
        fclose(fp);

    } else {
        FILE *fp = fopen("./alice.txt", "a");
        fprintf(fp, "log: %s\n", info);
        fclose(fp);
    }
}
...
void write_ping(char *info, int mode) {
    if (mode == 0) {
        FILE *fp = fopen("./alice_ping.txt", "w");
        fprintf(fp, "log: %s\n", info);
        fclose(fp);

    } else {
        FILE *fp = fopen("./alice_ping.txt", "a");
        fprintf(fp, "log: %s\n", info);
        fclose(fp);
    }
}
...
int main(int argc, char** argv) {
    ...
    int ping_count = 0;
    while (1) {
        int fd = open("./TheDomain/alice_RD", O_RDONLY);
        ...
        if (ret > 0) {
            ...
            if (buffer[0] == 3) {
                ...
                int fd_wr = open("./TheDomain/alice_WR", O_WRONLY);
                if (ping_count == 0) {
                    ping_count++;
                    write_ping("alice got ping from handler\n", write_mode);
                } else {
                    write_ping("alice got ping from handler\n", append_mode);
                }
                ...
                write_ping("alice send pong to handler\n", append_mode);
